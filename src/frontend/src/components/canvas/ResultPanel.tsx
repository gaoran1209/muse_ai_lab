...
export function ResultPanel({
  ...
  const [sortOverrides, setSortOverrides] = useState<Record<string, string[]>>({});
  ...
  const getSortedShots = useCallback(
    (lookId: string, lookShots: Shot[]): Shot[] => {
      const order = sortOverrides[lookId];
      if (!order) return lookShots;
      return [...lookShots].sort((a, b) => {
  ...
  const handleDragEnd = (lookId: string, lookShots: Shot[]) => {
    if (!dragShotId || !dragOverShotId || dragShotId === dragOverShotId) {
      setDragShotId(null);
      setDragOverShotId(null);
      return;
    }
    const sorted = getSortedShots(lookId, lookShots);
    const ids = sorted.map((s) => s.id);
    const fromIndex = ids.indexOf(dragShotId);
    const toIndex = ids.indexOf(dragOverShotId);
    ...
    newIds.splice(toIndex, 0, dragShotId);
    setSortOverrides((prev) => ({ ...prev, [lookId]: newIds }));
    setDragShotId(null);
    setDragOverShotId(null);
  };

  return (
    <div className={`result-panel-shell ${open ? 'is-open' : 'is-collapsed'}`}>
      <aside className={`result-panel ${open ? 'is-open' : 'is-hidden'}`}>
        ...
        <div className="result-panel-groups">
          {looks.map((look) => {
            ...
            const sortedShots = getSortedShots(look.id, lookShots);
                  ...
                  <>
                    <div className="result-thumbnails">
                      {sortedShots.map((shot) => (
                        <article
                          key={shot.id}
                          className={`result-thumb-card ${dragOverShotId === shot.id ? 'is-drag-over' : ''}`}
                          draggable
                          ...
                          onDragEnd={() => handleDragEnd(look.id, lookShots)}
                        >
                          <button
                            type="button"
      ...
      <button
        ...
        aria-expanded={open}
        aria-label={open ? '收起结果面板' : '展开结果面板'}
      >
        <span className="result-panel-toggle-icon">{open ? '›' : '‹'}</span>
        <span className="result-panel-toggle-label">结果面板</span>
      </button>
    </div>
  );
}
